...
const NEED_CONFIG: Record<NeedType, { label: string; className: string; dot: string }> = {
  water:     { label: "Water",     className: "bg-sky-50 dark:bg-sky-900/20 text-sky-700 dark:text-sky-400 border border-sky-200 dark:border-sky-800",                    dot: "bg-sky-500"    },
...
const STATUS_CONFIG: Record<RequestStatus, { label: string; className: string }> = {
  ...
  acknowledged:   { label: "Acknowledged",   className: "bg-primary-50 dark:bg-primary-900/20 text-primary-700 dark:text-primary-400 border border-primary-200 dark:border-primary-800" },
  ...
  cancelled:      { label: "Cancelled",      className: "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-600"               },
  false_positive: { label: "False Positive", className: "bg-slate-100 dark:bg-slate-700 text-slate-500 dark:text-slate-400 border border-slate-200 dark:border-slate-600"               },
};
...
const RISK_CONFIG: Record<RiskLevel, { label: string; className: string }> = {
  critical: { label: "Critical", className: "bg-red-100 dark:bg-red-900/30 text-red-800 dark:text-red-300 border border-red-300 dark:border-red-700 font-bold" },
...
const CONDITION_COLORS: Record<PatientCondition, string> = {
  stroke:       "bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 border border-red-200 dark:border-red-800",
  als:          "bg-purple-50 dark:bg-purple-900/20 text-purple-700 dark:text-purple-400 border border-purple-200 dark:border-purple-800",
  paralysis:    "bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 border border-orange-200 dark:border-orange-800",
  post_surgery: "bg-teal-50 dark:bg-teal-900/20 text-teal-700 dark:text-teal-400 border border-teal-200 dark:border-teal-800",
  elderly:      "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600",
  other:        "bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-600",
};
...
export const ConditionBadge: React.FC<ConditionBadgeProps> = ({ condition }) => (
  <span className={`badge ${CONDITION_COLORS[condition] ?? CONDITION_COLORS.other}`}>
